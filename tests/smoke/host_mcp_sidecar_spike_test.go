...
func (f *spikeFixture) report() {
	...
	builder.WriteString("-- timings (host monotonic clock) --\n")
...
func (f *spikeFixture) phaseReachabilityAndOwnership() {
	...
	f.pass("R1", fmt.Sprintf(
		"--network=host --cap-drop=ALL --read-only sidecar running as %s dialed the loopback-only sentinel at %s and returned the response nonce",
		f.identity, f.sentinel.address()))
